...
var _ = g.Describe("[sig-network][Suite:openshift/cluster-network-operator/disruptive] CNO", func() {
	var oc *CLI
	...
	// OCP-72817 / OCP-73205
	g.It("Make sure internalJoinSubnet and internalTransitSwitchSubnet is configurable post install as a Day 2 operation", func() {
		var (
			pod1Name          = "hello-pod1"
			pod2Name          = "hello-pod2"
			podLabel          = "hello-pod"
	...
	// OCP-51727
	g.It("ovsdb-server and northd should not core dump on node restart", func() {
	...
	// OCP-72028
	g.It("Join switch IP and management port IP for newly added node should be synced correctly into NBDB, pod on new node can communicate with old pod on old node", func() {
		ipStackType := checkIPStackType(oc)
		o.Expect(ipStackType).NotTo(o.BeEmpty())
